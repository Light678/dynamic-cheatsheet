...
def main(args: Arguments):
    ...
    for idx, example in enumerate(dataset):
        ...
        # --- FullHistory guard: limit history size to avoid 128k overflow ---
        if args.approach_name == "FullHistoryAppending":
            MAX_HISTORY = 150  # z.B. 100 / 150 / 200
            start = max(0, idx + 1 - MAX_HISTORY)
            corpus_slice = questions[start:idx + 1]
            emb_slice = embeddings[start:idx + 1] if embeddings is not None else None
        else:
            corpus_slice = questions[:idx + 1]
            emb_slice = embeddings[:idx + 1] if embeddings is not None else None
        ...
        output_dict = model.advanced_generate(
            ...
            original_input_corpus=corpus_slice,
            original_input_embeddings=emb_slice if args.approach_name in ["Dynamic_Retrieval",
                                                                          "DynamicCheatsheet_RetrievalSynthesis",
                                                                          "FullHistoryAppending"] else None,
